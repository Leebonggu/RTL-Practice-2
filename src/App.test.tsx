...
test('From order to order completion', async() => {
  ...
  // 주문환료 페이지
  // const loadingScreen = screen.getByText(/loading/i, { exact: false });
  ...
  const loadingDispappeared = screen.queryByText('loading');
  expect(loadingDispappeared).not.toBeInTheDocument();

  const firstPasgeButton = screen.getByRole('button', {
    name: '첫페이지로'
  });
  userEvent.click(firstPasgeButton);

  const productTotal = screen.getByText('상품 총 가격: 0');
